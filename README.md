...
운영 현황(`?manage`)의 **대회 찾기**에서 지역을 누르면 검색 결과가 새 창으로 열립니다.
검색어는 `js/config.js` 의 `EVENT_SOURCES` 가 만듭니다 — 유형(시·군청 공고 / 청소년수련관 /
청소년문화의집 / 지역 구분 없이) × 15개 시·군 × 네이버·구글.

**자동으로 긁어오지 않습니다.** 웹 검색 결과에는 후기·광고·타 지역 대회가 섞여서,
그대로 쌓으면 승인 대기열이 오히려 일거리가 됩니다. 검색어를 짜는 수고만 덜고
판단은 사람이 합니다.

기관 공고 RSS를 붙이면 훨씬 정확하지만, 시·군청 홈페이지가 대체로 RSS를 내놓지
않습니다(천안시청 확인). 피드를 실제로 찾은 기관만 나중에 하나씩 붙이는 게 낫습니다.
