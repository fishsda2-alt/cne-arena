...
├── scr.html                 스타크래프트 등록 — 래더 스크린샷을 브라우저 안에서 읽고 본인 확인 후 반영
├── css/style.css
├── js/
...
│   ├── scr.js               스타크래프트 등록 — 스크린샷 글자 인식(tesseract.js) + 확인 + 제출
│   ├── scr-parse.js         인식한 글자 → 등급·레이팅·승·패·래더 순위 추려내기
│   └── ranking.js           랭킹표 렌더링
...
│   ├── scr_report.py        스타크래프트 본인 등록 반영 · 같은 아이디면 갱신 · --remove 삭제
│   ├── check_scr_parse.js   스타크래프트 추려내기 시험 (node)
│   └── apps-script/
│       └── register-proxy.gs  등록·수정·삭제·대회·스타 중계 (구글 Apps Script에 붙여넣는 코드)
├── data/
│   ├── players.json         등록 선수 명단 (워크플로가 관리)
│   ├── ranking.json         자동 생성 — 사이트가 읽는 유일한 파일
│   ├── ranking.sample.json  API 키 없이 화면만 보고 싶을 때 쓰는 예시
│   ├── ranking.scr.json     스타크래프트 — 선수 본인이 올린 값 (롤과 분리)
│   ├── ranking.scr.sample.json  ?game=scr&sample 예시 화면용 가짜 데이터
...
│   ├── scr-from-site.yml      스타크래프트 본인 등록 반영 (+ 수동 실행으로 기록 삭제)
...
│   ├── check-parity.yml       아이콘 번호 대조 + 스타 추려내기 시험 (관련 파일 변경 시 자동)
...
- **API가 없는 종목**은 `selfReport: true` 로 둡니다 (스타크래프트가 그렇습니다).
  랭킹표가 다른 줄 모양으로 그려지고, 롤 등록 페이지의 종목 선택에서 빠지며,
  `registerUrl` 페이지에서 따로 받습니다. 표 머리글·요약 카드 문구는 `labels` 로 바꿉니다
  (적지 않은 문구는 롤 문구 그대로).
...
스타크래프트: 리마스터는 블리자드가 래더 정보를 외부에 열어 두지 않아 롤처럼 자동 수집할 수 없습니다.
그래서 **선수가 래더 화면 스크린샷으로 직접 확인한 값**을 올리는 방식입니다.
충남 지역 대회 팀 구성·클럽 활동에서 서로를 알아보는 용도이고, 결정 배경은
[docs/starcraft-remastered-검토.md](docs/starcraft-remastered-검토.md)에 있습니다.

```
scr.html → 스크린샷 고르기·붙여넣기 → 브라우저 안에서 글자 인식 → 입력칸 자동 채움
        → 선수가 스크린샷과 대조·확인 → 숫자만 전송 → Apps Script → Actions → 반영 (1~2분)
```

- **스크린샷은 어디에도 올라가지 않습니다.** 인식은 선수 브라우저 안에서 끝납니다
  (래더 화면에 배틀태그·친구 목록 같은 다른 사람 정보가 찍히기 때문).
- 처음 인식할 때 인식 데이터(약 8MB)를 한 번 받습니다. PC에서 10~20초 걸릴 수 있습니다.
- 인식은 **입력 보조**입니다. 틀린 칸은 선수가 고치고, "스크린샷과 같은지 확인했습니다"를
  체크해야 제출됩니다. 새 스크린샷을 올리면 체크가 풀립니다.
- 랭킹표 이름 옆에 **`본인 등록 / 09.14.`** 이 붙습니다 (마지막으로 올린 날).
  매일 갱신은 없고, 같은 게임 아이디로 다시 올리면 그 값으로 바뀝니다. 유효기간은 두지 않았습니다.
- **본인 인증은 없습니다.** 게임 아이디만 알면 덮어쓸 수 있으니 이상한 기록은 운영자가 지웁니다
  (아래 운영 방법 표).
- 클럽·지역 랭킹 페이지에도 스타크래프트 탭이 생기며, 평균은 레이팅으로 냅니다.
- 롤 랭킹표·롤 등록 흐름은 그대로입니다.

### 처음 한 번 — Apps Script 다시 배포 (필수)

`scripts/apps-script/register-proxy.gs` 에 스타크래프트 분기(`action: 'scr'`)가 추가됐습니다.
**다시 배포하기 전까지 스타크래프트 등록은 "Riot ID 형식이 올바르지 않습니다"로 거절됩니다.**

1. script.google.com 의 기존 프로젝트 → 코드 전체를 새 파일 내용으로 교체 → 저장
2. **배포 → 배포 관리 → 연필 → 버전: 새 버전 → 배포**
   (주소가 그대로 유지되므로 `js/config.js` 는 고칠 필요가 없습니다)
...
| 스타크래프트 잘못된 기록 지우기 | Actions → **스타크래프트 본인 등록** → Run workflow → 순번(`s001`) 또는 게임 아이디 |
...
| | 일반 등록 (`register.html`) | 스타크래프트 (`scr.html`) | 프로 지망 (`pro.html`) |
|---|---|---|---|
| 받는 정보 | 닉네임·지역·포지션·소속 | 게임 아이디·닉네임·지역·종족·소속 + 등급·레이팅·전적 | 이름·연락처·이메일 |
| 확인 방법 | 롤 프로필 아이콘 인증 | 선수 본인 확인 (인증 없음) | 운영자가 메일 확인 |
| 처리 방식 | **사이트에서 바로 자동 등록** | **사이트에서 바로 반영** | **이메일로만** |
| 결과 | 랭킹에 등재 | 스타크래프트 탭에 `본인 등록 / 날짜` 와 함께 등재 | 닉네임 옆 ★ 표시 |
| 보관 위치 | 저장소 (공개) | 저장소 (공개) — 스크린샷은 저장 안 함 | 구글 드라이브 (비공개) |
