...
`data/events.json` 의 `events` 에 한 칸 추가하면 랭킹 첫 화면에 뜹니다.
...
- **`end` 가 지나면 저절로 내려갑니다.** 지우는 걸 잊어도 지난 대회가 남지 않습니다.
- `game` 을 적으면 그 종목 탭에서만 보입니다. 비우면 모든 종목에서.
...
`js/config.js` 의 `discordUrl` 에 초대 링크를 넣으면 랭킹·소속 랭킹에 안내가 뜹니다.
...
대화는 디스코드에서 합니다. 소속 랭킹에서 팀을 누르면 팀원 명단이 펼쳐집니다.
