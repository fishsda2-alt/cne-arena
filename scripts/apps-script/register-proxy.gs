/**
 * 등록 폼 → GitHub Actions 중계 (Google Apps Script)
 *
...
/** 하루에 같은 IP에서 받을 수 있는 최대 신청 수 (도배 방지) */
var DAILY_LIMIT = 20;
...
function doPost(e) {
  try {
    ...
    if (code !== 204) {
      ...
      return fail('등록 서버에 전달하지 못했습니다. 잠시 후 다시 시도해 주세요.');
    }

    logSubmission(riotId, nickname);
    return ok();
  } catch (err) {
    console.error(err);
...
function logSubmission(riotId, nickname) {
  console.log('접수: ' + riotId + ' / ' + nickname);
}
